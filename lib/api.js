...
function mapAnimeFromDelivery(anime) {
  const poster =
...
function mapMangaFromDelivery(manga) {
  const cover =
...
export async function getAllAnime() {
  try {
    ...
    return animeList.map(mapAnimeFromDelivery);
  } catch (error) {
...
export async function getAnimeByGenre(genreUid) {
  try {
    ...
    const result = await query
      ...
      .find();
    const list = result[0] || [];
    return list.map(mapAnimeFromDelivery);
  } catch (error) {
...
export async function getAllManga() {
  try {
    ...
    return mangaList.map(mapMangaFromDelivery);
  } catch (error) {
...
export async function getMangaBySlug(slug) {
  try {
    ...
    if (matched) {
      return mapMangaFromDelivery(matched);
    }
...
export async function getHomepage() {
  try {
    ...
    if (homepage?.featured_anime?.length) {
      homepage.featured_anime = homepage.featured_anime.map((a) =>
        typeof a === 'object' && a ? mapAnimeFromDelivery(a) : a
      );
    }
    return homepage;
  } catch (error) {
